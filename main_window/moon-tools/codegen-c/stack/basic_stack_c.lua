...
function code_basic_cpp(names)
	...
	common_include_c();
	printfnl("");
printfnl("int %s_comp_node(%s *self,%s node1, %s node2);",names.class_name_lower,names.c_class_name,names.node_class_name,names.node_class_name);
	printfnl("");
...
printfnl("    X_MALLOC(self->data,%s,init_size);",names.node_class_name);
...
printfnl("    {");
printfnl("        REALLOC(self->data,%s,self->size,self->size * 2);",names.node_class_name);
