...
printfnl("");
printfnl("#define EXTRACT_SELF_POINTER(base,self)\\");
printfnl("    CONTAINER_OF(%s, self, base, base_task)\\",names.c_class_name);
printfnl("");
printfnl("static status_t %s_virtual_destroy(struct task *base)",names.class_name_lwr);
printfnl("{");
printfnl("    EXTRACT_SELF_POINTER(base,self);");
printfnl("    return %s_destroy(self);",names.class_name_lwr);
printfnl("}");
printfnl("");
printfnl("static status_t %s_virtual_run(struct task *base, uint32_t interval)",names.class_name_lwr);
printfnl("{    ");
printfnl("    EXTRACT_SELF_POINTER(base,self);");
...
printfnl("    self->base_task.run = %s_virtual_run;",names.class_name_lwr);
printfnl("    self->base_task.destroy = %s_virtual_destroy;",names.class_name_lwr);
printfnl("    return OK;");
printfnl("}");
printfnl("");
