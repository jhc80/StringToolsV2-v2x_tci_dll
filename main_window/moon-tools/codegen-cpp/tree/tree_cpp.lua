function code_cpp(names)
    ...
    printfnl("}");
	printfnl("");    
	printfnl("status_t %s::Copy(%s *p)",names.c_entry_class_name,names.c_entry_class_name);
	printfnl("{");
	printfnl("    return OK;");
	printfnl("}");
		printfnl("");
	printfnl("int %s::Comp(%s *p)",names.c_entry_class_name,names.c_entry_class_name);
	printfnl("{");
	printfnl("    ASSERT(p);");
	printfnl("    if(m_RawPtr == p->m_RawPtr)");
	printfnl("        return 0;");
	printfnl("    ASSERT(m_RawPtr);");
	printfnl("    ASSERT(p->m_RawPtr);");
