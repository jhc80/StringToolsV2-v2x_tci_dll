...
function code_service_function_declaration(info,class_prefix)
    ...
    if info.is_service then
        printf("status_t %s%s(CWebSocketMessage *_message)",
            class_prefix,
            func_name
        );    
    else
...
function code_h(idl_class)
    ...
    printf(long_text([[
        class %s:public CWebSocketServiceBase{
...
function code_single_param_idl(peer_name,info)
    ...
    printfnl("class %s:WebSocketParamBase{",class_name);
...
function common_cpp_include_name(name)
    return to_lower_underline_case("ws_"..name.."_types");
end
...
{
    CWebSocketServiceBase::InitBasic();
    return OK;
}
...
{
    this->InitBasic();
    CWebSocketServiceBase::Init(mgr);
...
function code_service_function(idl_class,info)
    ...
    if info.params then
        printf(long_text([[
            %s _param;
            _param.Init();
            ASSERT( _param.LoadBson(_message->GetData()) );]]
        ,8),param_class_name);
    end
    ...
    if not info.is_void then
        printf(long_text([[
            ///////////////
            CWebSocketCallContext _context;
            _context.Init();
            _context.FromMessage(_message); 
            ]],8)
        );    
...
function code_request_function(idl_class)
    ...
    printfnl("status_t %s::OnRequest(CWebSocketMessage *_message)",
        c_class_name(idl_class.name)
    );
    ...
    printnl("");
    printfnl("    int method = _message->GetMethod();");
    printnl("");
