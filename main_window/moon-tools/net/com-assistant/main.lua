...
local g_com = Serial.new();
if not g_com:Open(com_name) then
	printfnl("open file %s fail.",com_name);
	return
end

if not g_com:Configure(baud_rate,data_bits,stop_bits,parity) then
	printfnl("configure com %s fail.",com_name);
	return
end
...
function receive_thread(thread)
    ...
    while true do       		
		mem:SetSize(0);
		if g_com:Read(mem,mem:GetMaxSize()) > 0 then
