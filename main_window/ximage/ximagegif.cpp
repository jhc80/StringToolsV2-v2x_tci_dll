// xImageGIF.cpp: implementation of the CxImageGIF class.
//
//////////////////////////////////////////////////////////////////////
...
// struct_gifgce
//////////////////////////////////////////////////////////////////////
struct_gifgce::struct_gifgce()
{
    this->InitBasic();
}
int struct_gifgce::InitBasic()
{
    this->flags = 0;
    this->delaytime = 0;
...
int struct_gifgce::Copy(struct_gifgce *p)
{
    if(this == p) return OK;

    this->flags = p->flags;
    this->delaytime = p->delaytime;
    this->transpcolindex = p->transpcolindex;

    return OK;
}
int struct_gifgce::FromFile(CFileBase *pfile)
{
    pfile->Read(&this->flags,sizeof(BYTE));
    pfile->Read(&this->delaytime,sizeof(WORD));
    pfile->Read(&this->transpcolindex,sizeof(BYTE));
    return TRUE;
}
int struct_gifgce::ToFile(CFileBase *pfile)
{
    pfile->Write(&this->flags,sizeof(BYTE));
    pfile->Write(&this->delaytime,sizeof(WORD));
    pfile->Write(&this->transpcolindex,sizeof(BYTE));
    return TRUE;
}
int struct_gifgce::Size()
{
    int size = 0;
    size += sizeof(BYTE);
    size += sizeof(WORD);
...
// struct_dscgif
//////////////////////////////////////////////////////////////////////
struct_dscgif::struct_dscgif()
{
    this->InitBasic();
}
int struct_dscgif::InitBasic()
{   
    int i;
...
int struct_dscgif::Copy(struct_dscgif *p)
{
    int i;
    ...
    for(i = 0; i < 6; i++)
        this->header[i] = p->header[i];

    this->scrwidth = p->scrwidth;
    this->scrheight = p->scrheight;
    this->pflds = p->pflds;
    this->bcindx = p->bcindx;
    this->pxasrat = p->pxasrat;
    
    return OK;
}
int struct_dscgif::FromFile(CFileBase *pfile)
{
    pfile->Read(this->header,sizeof(char) *(6));
    pfile->Read(&this->scrwidth,sizeof(WORD));
    pfile->Read(&this->scrheight,sizeof(WORD));
    pfile->Read(&this->pflds,sizeof(char));
    pfile->Read(&this->bcindx,sizeof(char));
    pfile->Read(&this->pxasrat,sizeof(char));
    return TRUE;
}
int struct_dscgif::ToFile(CFileBase *pfile)
{
    pfile->Write(this->header,sizeof(char) *(6));
    pfile->Write(&this->scrwidth,sizeof(WORD));
    pfile->Write(&this->scrheight,sizeof(WORD));
    pfile->Write(&this->pflds,sizeof(char));
    pfile->Write(&this->bcindx,sizeof(char));
    pfile->Write(&this->pxasrat,sizeof(char));
    return TRUE;
}
int struct_dscgif::Size()
{
    int size = 0;
    size += sizeof(char) * 6;
    size += sizeof(WORD);
...
int struct_image::Copy(struct_image *p)
{
    if(this == p) return OK;
    this->l = p->l;
    this->t = p->t;
...
int struct_image::FromFile(CFileBase *pfile)
{
    pfile->Read(&this->l,sizeof(WORD));
    pfile->Read(&this->t,sizeof(WORD));
    pfile->Read(&this->w,sizeof(WORD));
    pfile->Read(&this->h,sizeof(WORD));
    pfile->Read(&this->pf,sizeof(BYTE));
    return TRUE;
}
int struct_image::ToFile(CFileBase *pfile)
{
    pfile->Write(&this->l,sizeof(WORD));
    pfile->Write(&this->t,sizeof(WORD));
    pfile->Write(&this->w,sizeof(WORD));
    pfile->Write(&this->h,sizeof(WORD));
    pfile->Write(&this->pf,sizeof(BYTE));
    return TRUE;
}
int struct_image::Size()
{
    int size = 0;
    size += sizeof(WORD);
    size += sizeof(WORD);
...
// CxImageGIF
//////////////////////////////////////////////////////////////////////
struct_TabCol::struct_TabCol()
{
    this->InitBasic();
}
int struct_TabCol::InitBasic()
{
    int i;
...
int struct_TabCol::Copy(struct_TabCol *p)
{
    if(this == p)return OK;
    //add your code
    return OK;
}
int struct_TabCol::FromFile(CFileBase *pfile)
{
    pfile->Read(&this->colres,sizeof(short));
    pfile->Read(&this->sogct,sizeof(short));
    
    int i;

    for(i = 0; i < 256; i++)
    {
        pfile->Read(&(this->paleta[i].r),sizeof(BYTE));
        pfile->Read(&(this->paleta[i].g),sizeof(BYTE));
        pfile->Read(&(this->paleta[i].b),sizeof(BYTE));
    }

    return TRUE;
}
int struct_TabCol::ToFile(CFileBase *pfile)
{
    pfile->Write(&this->colres,sizeof(short));
    pfile->Write(&this->sogct,sizeof(short));

    int i;

    for(i = 0; i < 256; i++)
    {
        pfile->Write(&(this->paleta[i].r),sizeof(BYTE));
        pfile->Write(&(this->paleta[i].g),sizeof(BYTE));
        pfile->Write(&(this->paleta[i].b),sizeof(BYTE));
    }

    return TRUE;
}
int struct_TabCol::Size()
{
    int size = 0;
    size += sizeof(short);
    size += sizeof(short);
...
// struct_RLE
//////////////////////////////////////////////////////////////////////
struct_RLE::struct_RLE()
{
    this->InitBasic();
}
int struct_RLE::InitBasic()
{
    this->rl_pixel = 0;
    this->rl_basecode = 0;
    this->rl_count = 0;
    this->rl_table_pixel = 0;
    this->rl_table_max = 0;
    this->just_cleared = 0;
    this->out_bits = 0;
    this->out_bits_init = 0;
    this->out_count = 0;
    this->out_bump = 0;
    this->out_bump_init = 0;
    this->out_clear = 0;
    this->out_clear_init = 0;
    this->max_ocodes = 0;
    this->code_clear = 0;
    this->code_eof = 0;
    this->obuf = 0;
...
int struct_RLE::Copy(struct_RLE *p)
{
    int i;

    if(this == p)return OK;

    this->rl_pixel = p->rl_pixel;
    this->rl_basecode = p->rl_basecode;
    this->rl_count = p->rl_count;
    this->rl_table_pixel = p->rl_table_pixel;
    this->rl_table_max = p->rl_table_max;
    this->just_cleared = p->just_cleared;
    this->out_bits = p->out_bits;
    this->out_bits_init = p->out_bits_init;
    this->out_count = p->out_count;
    this->out_bump = p->out_bump;
    this->out_bump_init = p->out_bump_init;
    this->out_clear = p->out_clear;
    this->out_clear_init = p->out_clear_init;
    this->max_ocodes = p->max_ocodes;
    this->code_clear = p->code_clear;
    this->code_eof = p->code_eof;
    this->obuf = p->obuf;
    this->obits = p->obits;

    for(i = 0; i < 256; i++)
        this->oblock[i] = p->oblock[i];
    
    this->oblen = p->oblen;
    
    return OK;
}
int struct_RLE::FromFile(CFileBase *pfile)
{
    pfile->Read(&this->rl_pixel,sizeof(int));
    pfile->Read(&this->rl_basecode,sizeof(int));
    pfile->Read(&this->rl_count,sizeof(int));
    pfile->Read(&this->rl_table_pixel,sizeof(int));
    pfile->Read(&this->rl_table_max,sizeof(int));
    pfile->Read(&this->just_cleared,sizeof(int));
    pfile->Read(&this->out_bits,sizeof(int));
    pfile->Read(&this->out_bits_init,sizeof(int));
    pfile->Read(&this->out_count,sizeof(int));
    pfile->Read(&this->out_bump,sizeof(int));
    pfile->Read(&this->out_bump_init,sizeof(int));
    pfile->Read(&this->out_clear,sizeof(int));
    pfile->Read(&this->out_clear_init,sizeof(int));
    pfile->Read(&this->max_ocodes,sizeof(int));
    pfile->Read(&this->code_clear,sizeof(int));
    pfile->Read(&this->code_eof,sizeof(int));
    pfile->Read(&this->obuf,sizeof(unsigned int));
    pfile->Read(&this->obits,sizeof(int));
    pfile->Read(this->oblock,sizeof(unsigned char) *(256));
    pfile->Read(&this->oblen,sizeof(int));

    return TRUE;
}
int struct_RLE::ToFile(CFileBase *pfile)
{
    pfile->Write(&this->rl_pixel,sizeof(int));
    pfile->Write(&this->rl_basecode,sizeof(int));
    pfile->Write(&this->rl_count,sizeof(int));
    pfile->Write(&this->rl_table_pixel,sizeof(int));
    pfile->Write(&this->rl_table_max,sizeof(int));
    pfile->Write(&this->just_cleared,sizeof(int));
    pfile->Write(&this->out_bits,sizeof(int));
    pfile->Write(&this->out_bits_init,sizeof(int));
    pfile->Write(&this->out_count,sizeof(int));
    pfile->Write(&this->out_bump,sizeof(int));
    pfile->Write(&this->out_bump_init,sizeof(int));
    pfile->Write(&this->out_clear,sizeof(int));
    pfile->Write(&this->out_clear_init,sizeof(int));
    pfile->Write(&this->max_ocodes,sizeof(int));
    pfile->Write(&this->code_clear,sizeof(int));
    pfile->Write(&this->code_eof,sizeof(int));
    pfile->Write(&this->obuf,sizeof(unsigned int));
    pfile->Write(&this->obits,sizeof(int));
    pfile->Write(this->oblock,sizeof(unsigned char) *(256));
    pfile->Write(&this->oblen,sizeof(int));
    return TRUE;
}
int struct_RLE::Size()
{
...
{
    this->StartUp(CXIMAGE_FORMAT_GIF);
...
void CxImageGIF::rle_write_block(struct_RLE* rle)
{
    ...
    g_outfile->Write(rle->oblock,rle->oblen);
    rle->oblen = 0;
}
...
void CxImageGIF::compressRLE( int init_bits, CFileBase* outfile)
{
...
BYTE CxImageGIF::GetPixelIndex(long x, long y)
{
...
BOOL CxImageGIF::EncodeRGB(CFileBase *fp)
{
    EncodeHeader(fp);
    EncodeComment(fp);
    ...
    for (long y=0;y<head.biHeight;y+=cellh){
        for (long x=0;x<head.biWidth;x+=cellw){
            ...
            if (w!=tmp.GetWidth() || h!=tmp.GetHeight()) 
                tmp.Create(w,h,8,0);
            ...
            for (long j=0;j<h;j++){
                for (long k=0;k<w;k++){
                    i=(BYTE)(1+k+cellw*j);
                    tmp.SetPaletteColor(i,GetPixelColor(x+k,head.biHeight-y-h+j,TRUE));
...
void CxImageGIF::EncodeHeader(CFileBase *fp)
{
    fp->Write("GIF89a",6);     //GIF Header
...
void CxImageGIF::Putword(int w, CFileBase *fp)
{
    fp->Putc((BYTE)(w & 0xff));
    fp->Putc((BYTE)((w >> 8) & 0xff));
}

void CxImageGIF::EncodeComment(CFileBase *fp)
{
    ...
    if (n) {
        ...
        fp->Putc((char)254);    //comment extension
        fp->Putc((BYTE)n);  //size of comment
        fp->Write(m_comment,n);
        fp->Putc(0);    //block terminator
    }
}


void CxImageGIF::EncodeExtension(CFileBase *fp)
{
    fp->Putc('!');
    fp->Putc((char)TRANSPARENCY_CODE);
...
void CxImageGIF::EncodeBody(CFileBase *fp, BOOL bLocalColorMap)
{
    curx = 0;
...
void CxImageGIF::compressNONE(int init_bits, CFileBase *outfile)
{
    register long c;
    register long ent;
...
void CxImageGIF::compressLZW(int init_bits, CFileBase *outfile)
{
    register long fcode;
    register long c;
    register long ent;
...
BOOL CxImageGIF::Encode(CFileBase * fp)
{
    fp->SetSize(0);
...
BOOL CxImageGIF::Encode(CFileBase *fp, CxImage **pImages, int pagecount, BOOL bLocalColorMap, BOOL bLocalDispMeth, int *running)
{
    cx_try {
        ...
        ghost.EncodeBody(fp,FALSE);
        for (i=1; i<pagecount; i++){
            if(running){if(!(*running))break;}
...
void CxImageGIF::EncodeLoopExtension(CFileBase *fp)
{
    fp->Putc('!');      //byte  1  : 33 (hex 0x21) GIF Extension code
    fp->Putc((char)255);        //byte  2  : 255 (hex 0xFF) Application Extension Label
    fp->Putc(11);       //byte  3  : 11 (hex (0x0B) Length of Application Block (eleven bytes of data to follow)
    fp->Write("NETSCAPE2.0",11);
...
{
    if (sz_comment_in) strcpy(m_comment,sz_comment_in); 
}

void CxImageGIF::GetComment(char* sz_comment_out)
{
    if (sz_comment_out) strcpy(sz_comment_out,m_comment); 
}
...
void CxImageGIF::GifMix(CxImage *imgsrc2, struct_image *imgdesc)
{
    ...
    long xmin = imgdesc->l;
    long xmax = min(GetWidth(), imgdesc->l + imgdesc->w);
...
BOOL CxImageGIF::Decode(CFileBase *fp)
{
    /* AD - for transparency */
    ...
    if (dscgif.pflds & 0x80)
        fp->Read(TabCol.paleta,sizeof(struct rgb_color)*TabCol.sogct);
    else 
    ...
    {
        if (fp->Read(&ch, sizeof(ch)) != sizeof(ch))
            break;
        ...
        {           
            ...
            case ',': // image
                ...
                if (image.pf & 0x80) {
                    ...
                    fp->Read(locpal,sizeof(struct rgb_color)*palcount);
                    pcurpal = locpal;
                }
                ...
                if (iImage==0){
                    ...
                    NEW(previousFrame,CxImage);
                    previousFrame->Copy(&backimage);
                ...
                fsize_t pos_start = fp->GetOffset();
                ...
                if (bTrueColor<2 ){ //standard GIF: mix frame with background
                    ...
                    backimage.SetPalette(GetPalette(),256);                 
                    ...
                    if (imaRGB==NULL)
                    {
                        NEW(imaRGB,CxImage(dscgif.scrwidth,dscgif.scrheight,24,CXIMAGE_FORMAT_GIF));
                    }

                    //copy the partial image into the full RGB image
                    for(long y=0;y<image.h;y++){
                        for (long x=0;x<image.w;x++){
                            imaRGB->SetPixelColor(x+image.l,dscgif.scrheight-1-image.t-y,GetPixelColor(x,image.h-y-1,TRUE),FALSE);
                        }
                    }
                }
                ...
                if (badcode){
                    ...
                    fp->Seek(fp->GetOffset() - (ibfmax - ibf - 1));
                }
                ...
                if (info.bGetAllFrames && imaRGB == NULL) {
                    ...
                    ASSERT(ppFrames);
                    NEW(ppFrames[iImage],CxImage);                  
                    ppFrames[iImage]->Copy(this);
...
int CxImageGIF::get_num_frames(CFileBase *fp, struct_TabCol *TabColSrc, struct_dscgif *dscgif)
{
    struct_image image;

    fsize_t pos=fp->GetOffset();
    int nframes=0;

    struct_TabCol TempTabCol;
    TempTabCol.Copy(TabColSrc);
    ...
    for (BOOL bContinue = TRUE; bContinue; )
    {
        if (fp->Read(&ch, sizeof(ch)) != sizeof(ch)) {break;}
        ...
        if (bPreviousWasNull || ch==0)
        {
            ...
            case ',': // image
                ...
                if (image.pf & 0x80) {
                    ...
                    ASSERT(3 == sizeof(struct rgb_color));
                    fp->Read(TempTabCol.paleta,sizeof(struct rgb_color)*TempTabCol.sogct);
...
short CxImageGIF::decoder(CFileBase *file, CImageIterator *iter, short linewidth, int &bad_code_count)
{
   ...
   /* Allocate space for the decode buffer */
    NEW_ARRAY(buf,BYTE,linewidth + 1);
...
int CxImageGIF::get_byte(CFileBase *file)
{
    if (ibf>=GIFBUFTAM){
        // FW 06/02/98 >>>
        ibfmax = (int)file->Read( buf , GIFBUFTAM) ;
...
fsize_t CxImageGIF::seek_next_image(CFileBase *fp, fsize_t position)
{
    fp->Seek(position);
    char ch1,ch2;
    ch1=ch2=0;
    while(fp->Read(&ch2,sizeof(char))>0){
        if (ch1 == 0 && ch2 == ','){
            fp->Seek(fp->GetOffset()-1);
            return fp->GetOffset();
        } else {
            ch1 = ch2;
        }
...
short CxImageGIF::get_next_code(CFileBase *file)
{
    short i, x;
    DWORD ret;
...
BOOL CxImageGIF::DecodeExtension(CFileBase *fp)
{
    BOOL bContinue;
    unsigned char count;
    unsigned char fc;
    
    bContinue = (sizeof(fc) == fp->Read(&fc, sizeof(fc)));
    if (bContinue) {
        /* AD - for transparency */
        if (fc == 0xF9) {
            bContinue = (sizeof(count) == fp->Read(&count, sizeof(count)));         
            if (bContinue) {
                ASSERT(gifgce.Size() == 4);
                gifgce.FromFile(fp);
                count = gifgce.Size();
                bContinue = (!fp->IsEnd());
        ...
        if (fc == 0xFE) 
        {
            bContinue = (sizeof(count) == fp->Read(&count, sizeof(count)));
            if (bContinue) {
                bContinue = (count == fp->Read(m_comment, count));
                m_comment[count]='\0';
        }   }
        ...
        if (fc == 0xFF) { //<DP> Application Extension block
            ...
            if (bContinue) {
                ...
                if (bContinue){
                    char AppID[11];
                    bContinue = (count == fp->Read(AppID, count));                  
                    if (bContinue) {
                        bContinue = (sizeof(count) == fp->Read(&count, sizeof(count)));
                        if (bContinue) {
                            BYTE *dati;
                            MALLOC(dati,BYTE,count);

                            bContinue = (dati!=NULL);
                            if (bContinue){                             
                                bContinue = (count == fp->Read(dati, count));
                                if (count>2){
        ...
        while (bContinue && fp->Read(&count, sizeof(count)) && count) {
            //log << "Skipping " << count << " bytes" << endl;
            fp->Seek(fp->GetOffset() + count);
        }
    }
    return bContinue;
}
