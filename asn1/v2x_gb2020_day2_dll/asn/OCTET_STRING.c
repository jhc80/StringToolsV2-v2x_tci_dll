...
OCTET_STRING__handle_control_chars(void *struct_ptr, const void *chunk_buf, size_t chunk_size) {
	...
	if(control_char >= 0) {
		...
		void *p = ASN_REALLOC(st->buf, st->size + 2);
		if(p) {
			st->buf = (uint8_t *)p;
...
static ssize_t OCTET_STRING__convert_hexadecimal(void *sptr, const void *chunk_buf, size_t chunk_size, int have_more) {
	...
	void *nptr = ASN_REALLOC(st->buf, new_size + 1);
	if(!nptr) return -1;
	st->buf = (uint8_t *)nptr;
	buf = st->buf + st->size;
...
OCTET_STRING_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
	...
	if(csiz->effective_bits >= 0) {
		...
		st->buf = (uint8_t *)ASN_MALLOC(st->size + 1);
	...
	do {
		...
		p = ASN_REALLOC(st->buf, st->size + len_bytes + 1);
		if(!p) RETURN(RC_FAIL);
		st->buf = (uint8_t *)p;
