...
INTEGER_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
	...
	if(ct) {
		if(ct->flags & APC_SEMI_CONSTRAINED) {
			...
			st->buf = (uint8_t *)ASN_MALLOC(1 + size + 1);
			if(!st->buf) ASN__DECODE_FAILED;
			st->size = size;
		}
	}
	...
	do {
		...
		p = ASN_REALLOC(st->buf, st->size + len + 1);
...
asn_umax2INTEGER(INTEGER_t *st, uintmax_t value) {
    ...
    buf = (uint8_t *)ASN_MALLOC(1 + sizeof(value));
    if(!buf) return -1;
...
asn_imax2INTEGER(INTEGER_t *st, intmax_t value) {
	...
	buf = (uint8_t *)(long *)ASN_MALLOC(sizeof(value));
	if(!buf) return -1;
