...
static int _el_addbytes(const void *buffer, size_t size, void *el_buf_ptr) {
    ...
    if(el_buf->length + size > el_buf->allocated_size) {
        ...
        p = ASN_REALLOC(el_buf->buf, new_size);
        if(p) {
            el_buf->buf = p;
...
SET_OF_encode_xer_callback(const void *buffer, size_t size, void *key) {
	...
	if(t->offset + size >= t->size) {
		...
		void *p = ASN_REALLOC(t->buffer, newsize);
		if(!p) return -1;
		t->buffer = p;
		t->size = newsize;
	}
...
SET_OF_encode_xer(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
	...
	if(xcan) {
		encs = (xer_tmp_enc_t *)ASN_MALLOC(list->count * sizeof(encs[0]));
...
SET_OF_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
    ...
    if(a && b) {
        ...
        asorted = ASN_MALLOC(a->count * sizeof(asorted[0]));
        bsorted = ASN_MALLOC(b->count * sizeof(bsorted[0]));
        if(!asorted || !bsorted) {
            FREEMEM(asorted);
            FREEMEM(bsorted);
            return -1;
